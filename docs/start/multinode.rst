...
.. note:: 
    ...
    From Ray 2.20, ``ray job submit`` or ``client = JobSubmissionClient("")`` is deprecated in current environment, and Ray version less than 2.40 is not compatible with current version of verl. We recommend you upgrade to Ray latest version and directly execute the training scripts.
...
If you encounter any issues in using AMD GPUs running verl, please contact `Yusheng Su <>`_.
