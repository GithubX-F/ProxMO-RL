...
class AlfredTWEnv(object):
    ...
    def collect_game_files(self, verbose=False):
        ...
        print(self.train_eval, data_path)
        ...
        # 添加初始检查
        print(f"\n[DEBUG] Scanning path: {data_path}")
        print(f"[DEBUG] Path exists: {os.path.exists(data_path)}")
        print(f"[DEBUG] Target task types: {task_types}\n")

        # 统计变量
        stats = {'total': 0, 'movable': 0, 'task_type': 0, 'no_game': 0, 'no_solvable': 0, 'unsolvable': 0, 'valid': 0}
        ...
        # 打印统计结果
        print(f"\n[DEBUG] Filter Statistics for {self.train_eval}:")
        print(f"  Total candidates:      {stats['total']}")
        print(f"  Filtered (movable):    {stats['movable']}")
        print(f"  Filtered (task_type):  {stats['task_type']}")
        print(f"  Filtered (no_game):    {stats['no_game']}")
        print(f"  Filtered (no_solvable):{stats['no_solvable']}")
        print(f"  Filtered (unsolvable): {stats['unsolvable']}")
        print(f"  Valid games:           {stats['valid']}\n")
