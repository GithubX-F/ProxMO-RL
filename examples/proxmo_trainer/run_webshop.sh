...
project_name='agentic_rl_webshop'
...
# WebShop 数据准备
TRAIN_FILE=${TRAIN_FILE:-"${RAY_DATA_HOME}/data/agent/webshop/${train_data_size}_${val_data_size}/train.parquet"}
TEST_FILE=${TEST_FILE:-"${RAY_DATA_HOME}/data/agent/webshop/${train_data_size}_${val_data_size}/test.parquet"}
...
save_freq=25  
test_freq=5
val_before_train=True  # WebShop通常在训练前验证

# 推理配置
temperature=0.4
do_sample=True
tensor_model_parallel_size=2
gpu_memory_utilization=0.6  # WebShop使用0.6

# 训练批次配置 - WebShop的原始设置
ppo_mini_batch_size=64  # 从256改为64
ppo_micro_batch_size_per_gpu=8  # 从32改为8
log_prob_micro_batch_size_per_gpu=16  # 从32改为16
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.logger=['console','hope','tensorboard'] \
