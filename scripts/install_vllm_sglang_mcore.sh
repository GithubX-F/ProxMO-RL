...
if [ $USE_SGLANG -eq 1 ]; then
    pip install "sglang[all]==0.4.6.post1" --no-cache-dir --find-links  && pip install torch-memory-saver --no-cache-dir
fi
...
if [ $USE_MEGATRON -eq 1 ]; then
    ...
    NVTE_FRAMEWORK=pytorch pip3 install --no-deps git+
    pip3 install --no-deps git+
fi
