...
class DataParallelPPOActor(BasePPOActor):
    ...
    def compute_gradients(self, data: DataProto, target_layer_ids: list = None):
        ...
        clip_ratio_low = self.config.clip_ratio_low  or clip_ratio
        clip_ratio_high = self.config.clip_ratio_high or clip_ratio
        ...
        # Build micro-batch iterator that still covers the whole batch
        if has_multi_modal_inputs:
            num_micro_batches  = batch.batch_size[0] // self.config.ppo_micro_batch_size_per_gpu
            micro_batches = data.select(select_keys, ["multi_modal_inputs"]).chunk(num_micro_batches)
        elif self.config.use_dynamic_bsz:
            max_tok_len = self.config.ppo_max_token_len_per_gpu * self.ulysses_sequence_parallel_size
            micro_batches, _  = rearrange_micro_batches(batch=batch, max_token_len=max_tok_len)
        else:
        ...
        for mb in micro_batches:
            if isinstance(mb, DataProto):
                mb = {**mb.batch.to(get_torch_device().current_device()), **mb.non_tensor_batch}
            else:
            ...
            entropy, log_prob = self._forward_micro_batch(micro_batch=mb, temperature=temperature, calculate_entropy=calculate_entropy)
            ...
            if calculate_entropy:
                ...
                # compute policy loss
                loss = pg_loss - entropy_loss * entropy_coeff
            else:
                loss = pg_loss

            if self.config.use_kl_loss:
                kld = kl_penalty(logprob=log_prob, ref_logprob=mb["ref_log_prob"], kl_penalty=self.config.kl_loss_type)
                kl_loss = agg_loss(loss_mat=kld, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
                loss = loss + kl_loss * self.config.kl_loss_coef
        ...
        # get flattened gradient vectors for all parameters
        full_grad_concat = None
        for p_name, p in self.actor_module.named_parameters():
            # p_name example: _fsdp_wrapped_module.model.layers.12._fsdp_wrapped_module._flat_param
            # exclude _fsdp_wrapped_module._flat_param
            layer_id = int(p_name.split("layers.")[1].split("._fsdp")[0]) if "layers." in p_name else -1
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    rewards = data["token_level_scores"] # token_level_scores
                    # advantage
