...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def get_remote_attr(self, dotted_name: str, call: bool = False, args=None, kwargs=None, _obj=None):
        
        args = args or ()
        kwargs = kwargs or {}
        _obj = self if _obj is None else _obj

        head, *tail = dotted_name.split(".", maxsplit=1)
        next_obj = getattr(_obj, head)

        if tail:
            # Recurse down the remaining path
            return self.get_remote_attr(
                tail[0], call=call, args=args, kwargs=kwargs, _obj=next_obj
            )

        if call and callable(next_obj):
            return next_obj(*args, **kwargs)
    ...
    def compute_gradients(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            flat_grads = self.actor.compute_gradients(data=data, target_layer_ids=data.meta_info["target_layer_ids"])
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After off-loading actor model during compute_gradients", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(self.actor_optimizer)
            log_gpu_memory_usage("After off-loading actor optimiser during compute_gradients", logger=logger)
